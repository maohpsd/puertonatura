...
function apollo13framework_pro_images_manager($meta){
	...
	array_splice( $meta['images_manager'], 6, 0, array(
			array(
				'name'        => __( 'Product this image represents', 'fatmoon' ),
				'description' => __( 'If you fill this, then on image you will see "add to cart" button.', 'fatmoon' ),
				'id'          => 'image_product_id',
				'default'     => '',
				'type'        => 'wp_dropdown_products',
			),
			array(
				'name'        => __( 'Proofing ID', 'fatmoon' ),
				'description' => __( 'If you enabled photo proofing and manual IDs then you can set it here.', 'fatmoon' ),
...
function apollo13framework_multi_upload_tags() {
	...
	if($apollo13framework_a13->check_for_valid_license()): ?>
        <p class="inline-part disabled" id="a13_multi_tags">
            <label for="a13-bulk-tagging"><?php esc_html_e( 'Add tags to selected items', 'fatmoon' ); ?></label>
            <input id="a13-bulk-tagging" class="newtag" size="16" value="" autocomplete="off" type="text" disabled="disabled" />
            <input class="button tagadd" value="<?php esc_attr_e( 'Add', 'fatmoon' ); ?>" type="button" disabled="disabled" />
        </p>
    <?php endif;
}
