...
function a13fe_widget_posts( $r, $instance, $type = 'normal' ) {
	while ( $r->have_posts() ) : $r->the_post();
		...
		if ( $type === 'popular' ) {
			...
			$comments_string = sprintf( __( '%s comment(s)', 'a13_framework_cpt' ), get_comments_number() );
	...
	class A13fe_Widget_Recent_Posts extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'classname'   => 'widget_recent_posts widget_about_posts',
			                     'description' => esc_html__( 'The most recent posts on your site', 'a13_framework_cpt' )
			);
			parent::__construct( 'recent-posts', esc_html__( 'Apollo13 - Recent Posts', 'a13_framework_cpt' ), $widget_ops );
		...
		function widget( $args, $instance ) {
			...
			$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Recent Posts', 'a13_framework_cpt' ) : $instance['title'], $instance, $this->id_base );
		...
		function form( $instance ) {
			...
			<p>
				<input id="<?php echo esc_attr( $this->get_field_id( 'content' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'content' ) ); ?>" type="checkbox" <?php checked( isset( $instance['content'] ) ? $instance['content'] : 0 ); ?> />&nbsp;<label for="<?php echo esc_attr( $this->get_field_id( 'content' ) ); ?>"><?php esc_html_e( 'Add posts excerpt', 'a13_framework_cpt' ); ?></label>
			</p>
			<?php
		}
	}
	...
	class A13fe_Widget_Popular_Posts extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'classname'   => 'widget_popular_entries widget_about_posts',
			                     'description' => esc_html__( 'The most popular posts on your site', 'a13_framework_cpt' )
			);
			parent::__construct( 'popular-posts', esc_html__( 'Popular Posts', 'a13_framework_cpt' ), $widget_ops );
		...
		function widget( $args, $instance ) {
			...
			$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Popular Posts', 'a13_framework_cpt' ) : $instance['title'], $instance, $this->id_base );
			if ( ! $number = absint( $instance['number'] ) ) {
				$number = 10;
			}
	...
	class A13fe_Widget_Related_Posts extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'classname'   => 'widget_related_entries widget_about_posts',
			                     'description' => esc_html__( 'Related posts to current post', 'a13_framework_cpt' )
			);
			parent::__construct( 'related-posts', esc_html__( 'Related Posts', 'a13_framework_cpt' ), $widget_ops );
		...
		function widget( $args, $instance ) {
			...
			$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Related Posts', 'a13_framework_cpt' ) : $instance['title'], $instance, $this->id_base );
			if ( ! $number = absint( $instance['number'] ) ) {
				$number = 10;
			}
