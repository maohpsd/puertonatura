...
class Apollo13Framework_Import extends WP_Importer {
	...
	function import_start( $file ) {
		if ( ! is_file($file) ) {
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'a13_framework_cpt' ) . '</strong><br />';
			echo esc_html__( 'The file does not exist, please try again.', 'a13_framework_cpt' ) . '</p>';
			die();
		}
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import category %s', 'a13_framework_cpt' ), esc_html($cat['category_nicename']) );
				echo '<br />';
				continue;
			}
		}
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import post tag %s', 'a13_framework_cpt' ), esc_html($tag['tag_name']) );
				echo '<br />';
				continue;
			}
		}
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import %1$s %2$s', 'a13_framework_cpt' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
				echo '<br />';
				continue;
			}
		}
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if($how_long > $this->max_time_running ){
				...
				printf( esc_html__( 'Importing posts for %s seconds.', 'a13_framework_cpt' ), esc_html( round( $how_long, 1 ) ) );
				echo '<br />';
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				...
				printf( esc_html__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'a13_framework_cpt' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				printf( esc_html__('%1$s &#8220;%2$s&#8221; already exists.', 'a13_framework_cpt'), esc_html( $post_type_object->labels->singular_name ), esc_html( $post['post_title'] ) );
				echo '<br />';
...
//				else
				...
				if ( is_wp_error( $post_id ) ) {
					/* translators: %1$s: post type singular name, 2$s: post_title */
					printf( esc_html__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'a13_framework_cpt' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( esc_html__( 'Failed to import %1$s %2$s', 'a13_framework_cpt' ), esc_html($taxonomy), esc_html($term['name']) );
							echo '<br />';
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			esc_html_e( 'Menu item skipped due to missing menu slug', 'a13_framework_cpt' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			...
			printf( esc_html__( 'Menu item skipped due to invalid menu slug: %s', 'a13_framework_cpt' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
	...
	function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments )
			return new WP_Error( 'attachment_processing_error',
				esc_html__( 'Fetching attachments is not enabled', 'a13_framework_cpt' ) );
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			return new WP_Error( 'attachment_processing_error', esc_html__('Invalid file type', 'a13_framework_cpt') );
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
					...
					sprintf(
						...
						__('Remote server returned %1$d %2$s for %3$s', 'a13_framework_cpt'),
						$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
			...
			return new WP_Error( 'import_file_error', esc_html__('Remote file is incorrect size', 'a13_framework_cpt') );
		}
		...
		if ( 0 == $filesize ) {
			...
			return new WP_Error( 'import_file_error', esc_html__('Zero size file downloaded', 'a13_framework_cpt') );
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new WP_Error( 'import_file_error', sprintf( esc_html__('Remote file is too large, limit is %s', 'a13_framework_cpt'), size_format($max_size) ) );
		}
