...
if(!function_exists('apollo13framework_make_scroller')){
	...
	function apollo13framework_make_scroller($args, $id = null ){
		...
		echo '<button class="scroll-below">'.esc_html__( 'Click here to scroll page below', 'fatmoon' ).'</button>';
		echo '</div>';
...
if(!function_exists('apollo13framework_cookie_message')){
	function apollo13framework_cookie_message() {
		...
		?>
		<div id="top-closable-message" class="top-message-container <?php esc_attr_e($position); ?>-screen">
...
function apollo13framework_new_proofed_albums(){
	...
	if($proof_option !== false && $proof_option !== '' && sizeof($proof_option) ){
		...
		echo '<strong>'.esc_html__( 'Your clients have marked following albums as finished in photo proofing process.', 'fatmoon' ).'</strong><br />';
		echo esc_html__( 'By using below links you can view and mark albums as "viewed", so they will not show up in this list again until new changes will be made.', 'fatmoon' );
		echo '</p>';
		echo '<ul>';
			...
			echo '<li><a href="'.esc_url(wp_nonce_url(add_query_arg( 'proof_viewed', $album_id, get_permalink($album_id) ), 'album-proof-viewed')).
			     ...
			     sprintf(
					/* translators: %s: human readable time in HTML tag */
				     esc_html__('&mdash; accepted %s ago', 'fatmoon' ),
		...
		echo '</ul>';
		echo '<p><strong><a id="hide-proof-notice" href="#">'. esc_html__('Mark all albums as viewed and hide message', 'fatmoon').'</a></strong></p>';
		echo '</div>';
	}
}
