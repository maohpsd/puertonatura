	...
	class A13fe_Widget_Recent_Albums extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'classname'   => 'widget_recent_albums widget_recent_cpt',
			                     'description' => esc_html__( 'Your most recent added albums', 'a13_framework_cpt' )
			);
			parent::__construct( 'recent-albums', esc_html__( 'Recent Albums', 'a13_framework_cpt' ), $widget_ops );
		...
		function widget( $args, $instance ) {
			...
			$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Recent Albums', 'a13_framework_cpt' ) : $instance['title'], $instance, $this->id_base );
		...
		function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show:', 'a13_framework_cpt' ); ?></label>
	...
	class A13fe_Widget_Recent_Works extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'classname'   => 'widget_recent_works widget_recent_cpt',
			                     'description' => esc_html__( 'Your most recent added works', 'a13_framework_cpt' )
			);
			parent::__construct( 'recent-works', esc_html__( 'Recent Works', 'a13_framework_cpt' ), $widget_ops );
		...
		function widget( $args, $instance ) {
			...
			$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Recent Works', 'a13_framework_cpt' ) : $instance['title'], $instance, $this->id_base );
			if ( ! $number = absint( $instance['number'] ) ) {
				$number = 10;
			}
		...
		class A13fe_Widget_Filter extends WP_Widget {
			...
			function __construct() {
				$widget_ops = array( 'classname'   => 'widget_filter',
				                     'description' => esc_html__( 'Filter Albums, Works or Posts', 'a13_framework_cpt' )
				);
				parent::__construct( 'filter', esc_html__( 'Filter', 'a13_framework_cpt' ), $widget_ops );
			...
			function widget( $args, $instance ) {
				...
				if ( $albums_list || $works_list || $posts_list ) {
					...
					if ( count( $terms ) ):
						$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Filter', 'a13_framework_cpt' ) : $instance['title'], $instance, $this->id_base );
						...
						echo '<li class="selected" data-filter="__all"><a href="' . esc_url( apollo13framework_current_url() ) . '"><i class="fa fa-square-o"></i>' . esc_html__( 'All', 'a13_framework_cpt' ) . '</a></li>';
			...
			function form( $instance ) {
				...
				$options = array(
					'blog'   => esc_html__( 'Blog - will appear in sidebar only if current page is main blog page.', 'a13_framework_cpt' ),
					'albums' => esc_html__( 'Albums - will appear in sidebar only if current page is albums list, or category of Albums.', 'a13_framework_cpt' ),
					'works' => esc_html__( 'Works - will appear in sidebar only if current page is works list, or category of Works.', 'a13_framework_cpt' )
				);
				?>
				<p><?php esc_html_e( 'Filter for:', 'a13_framework_cpt' ); ?></p>
				<?php
				...
				?>
				<hr />
				<p><em><?php esc_html_e( 'It filters by categories', 'a13_framework_cpt' ); ?></em></p>
