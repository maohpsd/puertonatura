...
class A13fe_Admin_Edit_Menu_Walker extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf(  esc_html__( '%s (Invalid)', 'a13_framework_cpt' ), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf(  esc_html__( '%s (Pending)', 'a13_framework_cpt' ), $item->title );
        }
    ...
    <dl class="menu-item-bar">
        <dt class="menu-item-handle">
            <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" style="<?php echo esc_attr($sub_menu_style); ?>"><?php esc_html_e( 'sub item', 'a13_framework_cpt' ); ?></span></span>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							<a href="<?php
                                ...
                                ?>" class="item-move-up"><abbr title="<?php echo esc_attr__( 'Move up', 'a13_framework_cpt' ); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
                                echo esc_url( wp_nonce_url(
                                    add_query_arg(
                                ...
                                ?>" class="item-move-down"><abbr title="<?php echo esc_attr__( 'Move down', 'a13_framework_cpt' ); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php echo esc_attr__( 'Edit Menu Item', 'a13_framework_cpt' ); ?>" href="<?php
                            ...
                            ?>"><span class="screen-reader-text"><?php esc_html_e( 'Edit', 'a13_framework_cpt' ); ?></span></a>
					</span>
        </dt>
    </dl>
    ...
    <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
        ...
        <p class="field-url description description-wide">
            <label for="edit-menu-item-url-<?php echo esc_attr( $item_id); ?>">
                <?php esc_html_e( 'URL', 'a13_framework_cpt' ); ?><br />
        ...
        <p class="description description-thin">
            <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e( 'Navigation Label', 'a13_framework_cpt' ); ?><br />
                <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
            </label>
        </p>
        <p class="description description-thin">
            <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e( 'Title Attribute', 'a13_framework_cpt' ); ?><br />
        ...
        <p class="field-link-target description">
            <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                ...
                <?php esc_html_e( 'Open link in a new window/tab', 'a13_framework_cpt' ); ?>
            </label>
        </p>
        <p class="field-css-classes description description-thin">
            <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e( 'CSS Classes (optional)', 'a13_framework_cpt' ); ?><br />
                <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
            </label>
        </p>
        <p class="field-xfn description description-thin">
            <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e( 'Link Relationship (XFN)', 'a13_framework_cpt' ); ?><br />
                <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
            </label>
        </p>
        <p class="field-description description description-wide">
            <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                <?php esc_html_e( 'Description', 'a13_framework_cpt' ); ?><br />
                <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'a13_framework_cpt' ); ?></span>
            </label>
        </p>
        ...
        <fieldset class="a13-theme-enhancement-options"><legend><?php esc_html_e( 'Theme enhancement options', 'a13_framework_cpt' ); ?></legend>
            <p class="field-a13_item_icon description">
                <label for="edit-menu-item-a13_item_icon-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Item icon', 'a13_framework_cpt' ); ?><br />
	        ...
	        <p class="field-a13_unlinkable">
		        <label for="edit-menu-item-a13_unlinkable-<?php echo esc_attr($item_id); ?>">
			        ...
			        <?php esc_html_e( 'Make this item normal text(will not be link)', 'a13_framework_cpt' ); ?>
		        </label>
	        </p>
        </fieldset>
        ...
        <fieldset class="a13-megamenu-options"><legend><?php esc_html_e( 'Mega menu options', 'a13_framework_cpt' ); ?></legend>
            <p class="field-enable-a13_mega_menu description level-0">
                <label for="edit-menu-item-a13_mega_menu-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( 'Enable mega menu?', 'a13_framework_cpt' ); ?>
                </label>
            </p>
            ...
            <p class="field-a13_mm_columns description level-0 if-mm-enabled">
                <label for="edit-menu-item-a13_mm_columns-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Number of columns', 'a13_framework_cpt' ); ?><br />
            ...
            <p class="field-a13_mm_remove_item level-1">
                <label for="edit-menu-item-a13_mm_remove_item-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( 'Don\'t show this item in mega menu. Show only descendants.', 'a13_framework_cpt' ); ?>
                </label>
            </p>

            <p class="field-a13_mm_html level-1">
                <label for="edit-menu-item-a13_mm_html-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Use HTML instead', 'a13_framework_cpt' ); ?><br />
                    ...
                    <span class="description"><?php esc_html_e('If filled, then this HTML will be used instead of classic menu option. Also no children options will be displayed.', 'a13_framework_cpt' ); ?></span>
                </label>
            </p>
        ...
        <p class="field-move hide-if-no-js description description-wide">
            <label>
                <span><?php esc_html_e( 'Move', 'a13_framework_cpt' ); ?></span>
                <a href="#" class="menus-move-up"><?php esc_html_e( 'Up one', 'a13_framework_cpt' ); ?></a>
                <a href="#" class="menus-move-down"><?php esc_html_e( 'Down one', 'a13_framework_cpt' ); ?></a>
                <a href="#" class="menus-move-left"></a>
                <a href="#" class="menus-move-right"></a>
                <a href="#" class="menus-move-top"><?php esc_html_e( 'To the top', 'a13_framework_cpt' ); ?></a>
            </label>
        </p>
        ...
        <div class="menu-item-actions description-wide submitbox">
            ...
            <p class="link-to-original">
                ...
                printf(  esc_html__( 'Original: %s', 'a13_framework_cpt' ), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' );
                ?>
            </p>
            <?php endif; ?>
            <a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr($item_id); ?>" href="<?php
                ...
                ) ); ?>"><?php esc_html_e( 'Remove', 'a13_framework_cpt' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
            ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Cancel', 'a13_framework_cpt' ); ?></a>
        </div>
