...
function a13fe_demo_data_download_files( $sublevel, &$sublevel_name, $demo_id ) {
	...
	if ( $file_to_download == '' && $revo_to_download == '' ) {
		...
		if ( is_writable( A13FRAMEWORK_IMPORTER_TMP_DIR .'/'.$demo_id ) ) {
			...
			return array( 'sublevel' => false, 'response' => array( 'code'=> 1013,'message' => A13FRAMEWORK_IMPORTER_TMP_DIR . '/' .$demo_id . '/' . $path . ' - ' . esc_html__( 'this folder has incorrect permissions set, the script is unable to write a file into it. Import stopped - please contact your hosting admin or Apollo13 support team.', 'a13_framework_cpt' ) ) );
		}
...
function a13fe_do_plugin_install( $plugin = array() ) {
	...
	$title = sprintf( esc_html__( 'Installing %s', 'a13_framework_cpt' ), $plugin['name'] );
	...
	if ( $install_type === 'update' ) {
		...
		/* translators: %s: plugin name */
		echo sprintf( esc_html__( 'Plugin updated: %s', 'a13_framework_cpt' ), esc_html( $plugin['name'] ) ) . "\r\n";
	} elseif ( $install_type === 'install' ) {
		$upgrader->install( $source );
		/* translators: %s: plugin name */
		echo sprintf( esc_html__( 'Plugin installed: %s', 'a13_framework_cpt' ), esc_html( $plugin['name'] ) ) . "\r\n";
	}
	...
	if ( $activate_plugin ) {
		...
		if ( is_wp_error( $activate ) ) {
			...
			/* translators: %s: plugin name */
			echo sprintf( esc_html__( 'Plugin activated: %s', 'a13_framework_cpt' ), esc_html(  $plugin['name'] ) ) . "\r\n";
		}
	}
...
function a13fe_demo_data_install_content( $sublevel, &$sublevel_name, $demo_id, $chosen_options ) {
	...
	if ( class_exists( 'Apollo13Framework_Import' ) ) {
		...
		if ( ! $is_done ) {
			...
			$sublevel        = esc_html__( 'While importing content...', 'a13_framework_cpt' );
			/* translators: %s: post ID */
			$sublevel_name   = sprintf( esc_html__( 'Imported %1$d of %2$d items', 'a13_framework_cpt' ), $last_item, $all_items ) . "\r\n";
