...
function a13fe_get_demo_exporter_content(){
	...
	?>
	<h2><?php echo esc_html__( 'Export &amp; Import theme options', 'a13_framework_cpt' ); ?></h2>
	<p style="text-align: center;"><a href="<?php echo esc_url( $apollo13framework_a13->get_docs_link('export') ); ?>"><?php echo esc_html__( 'Check the documentation for instructions about using Export Area.', 'a13_framework_cpt' ); ?></a></p>
	<label for="export_theme_options_field"><?php echo esc_html__( 'Export theme settings', 'a13_framework_cpt' ); ?></label>
	...
	<button class="button button-secondary copy-content" type="submit"><?php echo esc_html__( 'Copy to clipboard', 'a13_framework_cpt' ); ?></button>

	<hr />

	<label for="import_theme_options_field"><?php echo esc_html__( 'Import theme settings', 'a13_framework_cpt' ); ?></label>
	<textarea rows="10" cols="20" class="large-text" id="import_theme_options_field"></textarea>
	<button class="button button-primary import-theme-settings" data-import-field="import_theme_options_field" type="submit"><?php echo esc_html__( 'Import theme settings', 'a13_framework_cpt' ); ?></button>
	<div class="attention"><?php echo esc_html__( 'Attention! It will overwrite your current theme settings.', 'a13_framework_cpt' ); ?></div>
	...
	if ( apollo13framework_is_home_server() ){
		?>
		<hr />
		<label for="export_options_field"><?php echo esc_html__( 'Export demo data options(site_config file)', 'a13_framework_cpt' ); ?></label>
...
function a13fe_import_theme_settings() {
	...
	if( strlen( $settings ) ){
		...
		if( ! is_null( $settings ) ){
			...
			$out['message'] = esc_html__( 'Import successful!', 'a13_framework_cpt' );
		}
		else{
			$out['response'] = 'error';
			$out['message']  = esc_html__( 'Looks like malformatted JSON string, cannot proceed with the import.', 'a13_framework_cpt' );
		}
	}
	else{
		$out['response'] = 'error';
		$out['message']  = esc_html__( 'Nothing to import.', 'a13_framework_cpt' );
	}
