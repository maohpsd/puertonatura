...
class Apollo13Framework
{
    ...
    function start()
    {
        ...
        define('A13FRAMEWORK_THEME_ID_NUMBER', '15966469');
        define('A13FRAMEWORK_OPTIONS_NAME', 'apollo13_option');
        define('A13FRAMEWORK_CACHE', 'apollo13_fatmoon_cache');
        define('A13FRAMEWORK_THEME_VERSION', '2.3.1');
...
//		define('A13FRAMEWORK_CUSTOM_POST_TYPE_WORK_SLUG', 'work'); //just to show it also exist - defined in real a bit lower
    ...
    function setup()
    {
        ...
        register_nav_menus(array(
            'header-menu' => __( 'Site Navigation', 'fatmoon' ),
            'top-bar-menu' => __( 'Alternative short top bar menu', 'fatmoon' ),
        ));
    }
    ...
    function check_for_warnings()
    {
        ...
        if ($r->have_posts() && strlen(A13FRAMEWORK_CUSTOM_POST_TYPE_ALBUM_SLUG) ) {
            /* translators: %s: URL */
            $notices['albums-slug'] = sprintf( __( 'Warning - slug for Albums is taken by page or post! It may cause problems with displaying albums. Edit slug of <a href="%s">this post</a> to make sure everything works good.', 'fatmoon' ), esc_url( site_url( '/' . A13FRAMEWORK_CUSTOM_POST_TYPE_ALBUM_SLUG ) ) );
        }
        ...
        if ($r->have_posts() && strlen(A13FRAMEWORK_CUSTOM_POST_TYPE_WORK_SLUG)) {
            /* translators: %s: URL */
            $notices['works-slug'] = sprintf( __( 'Warning - slug for Works is taken by page or post! It may cause problems with displaying works. Edit slug of <a href="%s">this post</a> to make sure everything works good.', 'fatmoon' ), esc_url( site_url( '/' . A13FRAMEWORK_CUSTOM_POST_TYPE_WORK_SLUG ) ) );
        }
        // Reset the global $the_post as this query have stomped on it
        wp_reset_postdata();
...
//            'images_manager' //no parent options here
    ...
    function is_companion_plugin_ready($fail_message = false, $silent = false){
        ...
        if ( empty( $plugins[$plugin_file] ) ) {
            ...
            $label = __( 'Install Apollo13 Framework Extensions plugin', 'fatmoon' );
        }
        ...
        elseif ( is_plugin_inactive( $plugin_path ) ){
            ...
            $label = __( 'Activate Apollo13 Framework Extensions plugin', 'fatmoon' );
        }
        ...
        elseif( version_compare( $plugins[$plugin_file]['Version'], A13FRAMEWORK_MIN_COMPANION_VERSION, '<'  ) ){
            ...
            $label = __( 'Update Apollo13 Framework Extensions plugin', 'fatmoon' );
        }

        if(!$ready && !$silent){
            $message = $fail_message ? $fail_message : __( 'This feature requires Apollo13 Framework Extensions plugin to be active and in proper version.', 'fatmoon' );
