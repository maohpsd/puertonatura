...
function a13fe_scroller_shortcode( $atts ) {
	...
	//without id
	if ( $id === false ) {
		return esc_html__( 'Error: No post id was given for scroller shortcode.', 'a13_framework_cpt' );
	}

	//it doesn't contain items to use
	$has_images = strlen( get_post_meta( $id, '_images_n_videos', true ) ) > 0;
	if ( ! $has_images ) {
		return esc_html__( 'Error: Selected post does not contain any items to use in shortcode. Use proper album or work.', 'a13_framework_cpt' );
	}
