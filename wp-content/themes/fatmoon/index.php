...
//Lets decide what is the title
	if ( is_search() ) {
		...
		$_title = sprintf( esc_html__( '%1$d Search results for "%2$s"', 'fatmoon' ), $count, get_search_query() );
	} elseif ( is_archive() ) {
		if ( is_author() ) {
			/* translators: %s - author name */
			$_title = sprintf( esc_html__( 'Author Archives: %s', 'fatmoon' ), "<span class='vcard'>" . get_the_author() . "</span>" );
		} elseif ( is_category() ) {
			/* translators: %s - category name */
			$_title = sprintf( esc_html__( 'Category Archives: %s', 'fatmoon' ), '<span>' . single_cat_title( '', false ) . '</span>' );
		} elseif ( is_tag() ) {
			/* translators: %s - tag name */
			$_title = sprintf( esc_html__( 'Tag Archives: %s', 'fatmoon' ), '<span>' . single_tag_title( '', false ) . '</span>' );
		} elseif ( is_day() ) {
			/* translators: %s - day */
			$_title = sprintf( esc_html__( 'Daily Archives: %s', 'fatmoon' ), '<span>' . get_the_date() . '</span>' );
		} elseif ( is_month() ) {
			/* translators: %s - month */
			$_title = sprintf( esc_html__( 'Monthly Archives: %s', 'fatmoon' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );
		} elseif ( is_year() ) {
			/* translators: %s - year */
			$_title = sprintf( esc_html__( 'Yearly Archives: %s', 'fatmoon' ), '<span>' . get_the_date( 'Y' ) . '</span>' );
		} else {
			$_title = esc_html__( 'Blog Archives', 'fatmoon' );
		}
	}
