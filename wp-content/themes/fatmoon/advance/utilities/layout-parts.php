...
if(!function_exists('apollo13framework_search_form')){
	...
	function apollo13framework_search_form(
        ...
        $field_search = '<input' .
            ' placeholder="' . esc_attr__('Search &hellip;', 'fatmoon' ) . '" ' .
        ...
        $form = '
                <form class="search-form" role="search" method="get" action="' . esc_url( home_url( '/' ) ) . '" >
                    <fieldset class="semantic">
                        ' . $field_search . '
                        <input type="submit" id="searchsubmit' . esc_attr( $search_id ) . '" title="'. esc_attr__( 'Search', 'fatmoon' ) .'" value="'. esc_attr__( 'Search', 'fatmoon' ) .'" />
			        ...
			        <div id="post-<?php the_ID(); ?>" <?php post_class('content-box'); ?>>
				        <div class="formatter">
				            ...
				            <div class="real-content">
				                ...
				                <?php
				                wp_link_pages( array(
				                        'before' => '<div id="page-links">'.esc_html__( 'Pages: ', 'fatmoon' ),
				                        'after'  => '</div>')
				                );
				                ?>
				            </div>
