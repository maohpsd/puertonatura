...
function apollo13framework_print_form_controls( $option, &$params, $is_meta = false ) {
	...
	if ( $option['type'] == 'upload' ) {
		$upload_button_text = ! empty( $option['button_text'] ) ? $option['button_text'] : esc_html__( 'Upload', 'fatmoon' );
		?>
		...
		<div class="upload-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				?> />
				<input id="clear_<?php echo esc_attr($input_prefix . $option['id']); ?>" class="clear-image-button button" type="button" value="<?php echo esc_attr__( 'Clear field', 'fatmoon' ) ?>" />
	...
	elseif ( $option['type'] == 'color' ) {
		...
		<div class="color-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				<?php apollo13framework_input_help_tip( __( '<p>Use valid CSS <code>color</code> property values( <code>green, #33FF99, rgb(255,128,0)</code> ), or get your color with color picker tool.</p><p>Use <code>Transparent</code> button to insert transparent value.</p><p>Left empty to use default theme value.</p>', 'fatmoon' ) ); ?>
				...
				<button class="transparent-value button-secondary"><?php esc_html_e( 'Transparent', 'fatmoon' ); ?></button>
				<p class="desc"><?php echo wp_kses( $description, $valid_tags ); ?></p>
			</div>
		</div>
		<?php
		...
		<div class="tag_media-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				<p><input id="<?php echo esc_attr($input_prefix . $option['id']); ?>-helper" class="not-to-collect newtag" size="16" value="" type="text">
					<input class="not-to-collect button tagadd" value="<?php esc_attr_e( 'Add', 'fatmoon' ); ?>" type="button"></p>
	...
	elseif ( $option['type'] == 'slider' ) {
		...
		<div class="slider-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				<?php apollo13framework_input_help_tip( __( 'Use slider to set proper value. You can click on slider handle and then use arrows keys(on keyboard) to adjust value precisely. You can also type in input value that is in/out of range of slider, and it will be used.', 'fatmoon' ) ); ?>
		...
		<div class="select-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				if ( $products ) {
					...
					echo '<option value="0">'.esc_html__( 'None', 'fatmoon' ).'</option>';
					...
					echo '<span class="empty-type">'. esc_html__( 'There are no products yet!', 'fatmoon' ) . '</span>';
				}
